    ...
    <div className="flex flex-col gap-4 md:gap-6">
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className={`grid gap-3 ${isMobile ? 'grid-cols-2' : 'grid-cols-1'}`}>
            {[
              { label: 'Troféus Clã', val: '0', color: 'text-white', status: 'Bloqueado' },
              { label: 'Próx Nv', val: '0', color: 'text-gaming-purple', status: 'Pendente' }
            ].map((item) => (
              <div key={item.label} className={`relative group overflow-hidden bg-gaming-card border border-gaming-border p-3 md:p-4 rounded-xl flex flex-col gap-1 hover:border-white/20 transition-all shadow-lg ${isEcoMode ? '' : 'backdrop-blur-sm'}`}>
                  <div className="flex justify-between items-start">
                    ...
                    <div className="px-1 py-0.5 bg-white/5 border border-white/10 rounded text-[5px] font-black uppercase text-white/20 tracking-widest">Em Dev</div>
                  </div>
                  <div className="flex flex-col truncate">
                    <span className="text-[7px] md:text-[9px] uppercase text-white/40 font-bold tracking-widest truncate">{item.label}</span>
                    <span className="text-[6px] md:text-[8px] text-white/10 font-bold tracking-wider uppercase truncate italic">{item.status}</span>
                  </div>
                  {!isEcoMode && (
                    <div className="absolute inset-0 bg-linear-to-tr from-white/5 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
                  )}
              </div>
            ))}
        </div>
